...
def viterbi_3(train, test):
    ...
    suffixes = ["s","ly", "ed","ing","ity","ty","ful"]
    ...
    hapax_words = []
    for word in single_words_count:
        if single_words_count[word] == 1:
    ...
    for tag in tags_solo:
        htags = 0
        stags = Counter()
        for h in hapax_words:
            htags += words_n_tags[(h, tag)]
            substring1 = h[-1:]
            substring2 = h[-2:]
            substring3 = h[-3:]
            if substring1 in suffixes:
                stags[substring1] += words_n_tags[(h, tag)]
            elif substring2 in suffixes:
                stags[substring2] += words_n_tags[(h, tag)]
            elif substring3 in suffixes:
                stags[substring3] += words_n_tags[(h, tag)]
        hapax_tags[tag] = (htags + laplace) / (hapax_count + laplace * unique_tags_count)
        for sub in suffixes:
            suffix_tags[(tag, sub)] = (stags[sub] + laplace) / (hapax_count + laplace * unique_tags_count)
    #print(suffix_tags["NOUN",'ly'])
        ...
        for each_word in sentence:
            if curr_word_index == 0:
                for tag_idx in range(0, unique_tags_count):
                    ...
                    substring1 = each_word[-1:]
                    substring2 = each_word[-2:]
                    substring3 = each_word[-3:]
                    if substring1 in suffixes:
                        laplace_hapax = suffix_tags[curr_tag, substring1] * laplace
                    elif substring2 in suffixes:
                        laplace_hapax = suffix_tags[curr_tag, substring2] * laplace
                    elif substring3 in suffixes:
                        laplace_hapax = suffix_tags[curr_tag, substring3] * laplace
                    else:
                        laplace_hapax = hapax_tags[curr_tag] * laplace
                    p_tag = np.log((tags_solo[curr_tag] + laplace_hapax) / (total_words + laplace_hapax * (unique_words_count + 1)))

                    prob_word = np.log(
                        (words_n_tags[(each_word, curr_tag)] + laplace_hapax) / (
                                tags_solo[curr_tag] + laplace_hapax * (unique_words_count + 1)))
                ...
                for tag_idx in range(0, unique_tags_count):
                    ...
                    for prev_tag_idx in range(0, unique_tags_count):
                        ...
                        if substring1 in suffixes:
                            ...
                            laplace_hapax = hapax_tags[curr_tag] * laplace
                        prob_tag = np.log(
                            (tag_pairs[(prev_tag, curr_tag)] + laplace_hapax) / (
                                    tags_solo[prev_tag] + laplace_hapax * unique_tags_count))
